...
app.get("/rs/rs/containers/get", function(req, res) {
    db.all("SELECT * FROM containers ORDER BY name ASC", function(err, rows) {
        res.json(rows);
    });
});

app.get("/rs/rs/container/:id/stats/latest", function(req, res) {
    db.all("SELECT * FROM stats WHERE id = ? ORDER BY ts DESC LIMIT 5", req.params.id, function(err, rows) {
        res.json(rows);
    });
});

app.get("/rs/rs/container/:id/:chart/:zoom", function(req, res) {
...
app.get("/rs/rs/all/:chart/:zoom", function(req, res) {
    var chart = req.params.chart;
    var zoom = req.params.zoom;
    if (!isValidChart(chart) || !isValidZoom(zoom)) {
        return res.json([]);
